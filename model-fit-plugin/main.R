...
load(file.path(R_SWS_SHARE_PATH,'food-loss-test/Inputs/input_data2.RData'))
load(file.path(R_SWS_SHARE_PATH,'food-loss-test/Inputs/FAOCrops2.RData'))
load(file.path(R_SWS_SHARE_PATH,'food-loss-test/Inputs/CropCalendar2.RData'))
...
# the only "M49 numeric" key we need (matches geographicaream49 in original model_data)
M49[, m49_numeric := as.numeric(m49_country_code)]


write_excel_csv(M49,file=file.path(R_SWS_SHARE_PATH,"food-loss-test/M49.csv"))
...
key_prod = DatasetKey(
  ...
  dimensions = list(
    ...
    Dimension(name = "measuredItemCPC",   keys = as.character(unique(FAOCrops$measureditemcpc))), # optional
...
mean(is.na(harvest_calendar_imputed$harvesting_month_end))
...
# Filter to your modelling years 
year_min <- 1991
year_max <- 2024
...
save_dir_model_data <- file.path(R_SWS_SHARE_PATH, "food-loss-test", "Model_data")
...
ggsave(gridExtra::arrangeGrob(jamaica_gdp,lith_lpi,nrow=1),file=file.path(R_SWS_SHARE_PATH,"food-loss-test/Plots/covariates_example.png"),width=8,height=3.25,dpi=600)
...
# 3) now nimble should load on workers
clusterEvalQ(par_cluster, { library(nimble); NULL })


# Export helper functions/objects needed on workers
...
save_dir <- file.path(R_SWS_SHARE_PATH, "food-loss-test", "Saved_models", "mcmc_outputs_2026")
...
ggsave(stage_method_plot,file=file.path(R_SWS_SHARE_PATH,"food-loss-test/Plots/stage_method_plot.png"),width=8,height=3.75,dpi=600)
...
ggsave(food_group_coefficients,file=file.path(R_SWS_SHARE_PATH,"food-loss-test/Plots/food_group_coefficients.png"),width=8,height=5,dpi=600)
...
write_excel_csv(variance_explained_medians,file=file.path(R_SWS_SHARE_PATH,"food-loss-test/Tables/variance_explained.csv"))

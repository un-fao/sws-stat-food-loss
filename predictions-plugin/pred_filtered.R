...
#When running on her computer, Lyuba needed to choose a lower number of iterations, therefore
#n_sim_fit is equal to 400 and she run the predictions part of code with n_sim_pred=40


save_dir_post <- file.path(R_SWS_SHARE_PATH,"food-loss-test","Saved_models","mcmc_outputs_2026")
save_dir_data <- file.path(R_SWS_SHARE_PATH,"food-loss-test","Model_data")
...
###We will need to consider only valid production*country*year combinations
setDT(prod_support)
prod_support[, `:=`(
...
dir_country <- file.path(R_SWS_SHARE_PATH,"food-loss-test","Working_samples","COUNTRY")
...
dir_country  <- file.path(R_SWS_SHARE_PATH,"food-loss-test","Working_samples","COUNTRY")
out_dir_sua  <- file.path(R_SWS_SHARE_PATH,"food-loss-test","Working_samples","SUA")
out_dir_surv <- file.path(R_SWS_SHARE_PATH,"food-loss-test","Working_samples","Survey")
...
save(
    ...
    file = file.path(R_SWS_SHARE_PATH,"food-loss-test/Saved_models/full_prediction_quantiles_df_2026.RData")
)
...
save(
    ...
    file = file.path(R_SWS_SHARE_PATH,"food-loss-test/Saved_models/full_prediction_quantiles_SUA_df_2026.RData")
)
...
save(
    ...
    file = file.path(R_SWS_SHARE_PATH,"food-loss-test/Saved_models/full_prediction_quantiles_survey_df_2026.RData")
)
...
M49 <- as.data.table(M49)  # contiene iso3 + m49_numeric

setDT(full_prediction_quantiles_df)
full_prediction_quantiles_df[M49[, .(iso3, m49_numeric)], on="iso3", m49_numeric := i.m49_numeric]

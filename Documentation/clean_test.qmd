...
$$
with $$\begin{aligned} \mu_i &= \sum_{p=1}^P (a1_p + a2_p (region_i) + a3_p (subregion_i) + b1_p (foodgroup_i)) X_{i,p}\\
&+ \sum_{p=1}^2 (a4_p (country_i) + b2_p (product_i) + b3_p (country_i, foodgroup_i) + b4_p (country_i, product_i) X_{i,p}\\ 
&+ c1(stage_i) + c2(datamethod_i) + c3(source_i) \end{aligned}$$
...
The `prediction_meta` object stores at least:
...
At this point, the prediction stage has access to the country grouping structure, the product support and metadata carried by `fbsTree1`, the imputed harvest calendar, the covariates, and the production support used to restrict valid combinations. The production support table is normalized to the required variable types and keyed by:
...
- the product metadata from `fbsTree1`;
...
This step is essential. Without it, the output datasets would contain rows for unsupported combinations that were never intended to be part of the final saved estimates.
